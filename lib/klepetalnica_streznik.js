...
//predlozena funkcija za obdelovanje dregljaja
function obdelajDregljaj(socket) {
  socket.on('dregljaj', function (dregljaj) {
    console.log("Server: dregljaj prispel");
    if (dregljaj.vzdevek) {
      console.log("Server: Dregljaj za: "+dregljaj.vzdevek);
      var socketIdNaslovnika = null;
      for (var id in vzdevkiGledeNaSocket) {
       if (dregljaj.vzdevek == vzdevkiGledeNaSocket[id]) {
         socketIdNaslovnika = id;
         console.log("Server: dobil naslovnika: " + socketIdNaslovnika);
         break;
       }
      }
